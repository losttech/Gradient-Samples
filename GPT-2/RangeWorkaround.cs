namespace Gradient.Samples.GPT2
{
    ...
    static class RangeWorkaround
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static int ToPythonIndex(Index index) => index.FromEnd ? ~index.Value : index.Value;

        public static PyObject All() => Pythonify(Range.All());
        public static PyObject FromStart(Index start) => Pythonify(Range.FromStart(start));
        public static PyObject ToEnd(Index end) => Pythonify(Range.ToEnd(end));
    }
}
